...
Дополнение и ответ на вопрос 
Почему Redis блокирует операции записи (Latency generated by expires):

Операции по истечению срока хранения в Redis выполняются двумя способами: ленивым и активным. Ленивый способ заключается в том, чтобы 
удалять ключи, которые запрашиваются командой и при этом уже истекли. Активный способ заключается в том, чтобы удалять несколько ключей 
каждые 100 миллисекунд. Активный способ удаления ключей адаптивен: каждые 100 миллисекунд запускается процесс, который выбирает 
некоторое количество ключей, проверяет, истек ли их срок, и удаляет те, что уже просрочены. Если больше 25% выбранных ключей 
оказываются просроченными, процесс повторяется. По умолчанию количество проверяемых ключей в одном цикле равно 20, и процесс 
выполняется 10 раз в секунду, так что обычно активно удаляется всего 200 ключей в секунду. Этого достаточно, чтобы быстро очищать базу 
данных, даже если просроченные ключи долгое время не запрашивались и не удалялись ленивым способом. Однако, если в одной и той же 
секунде истекает 25% или больше всех выбранных ключей, Redis может заблокироваться, чтобы уменьшить процент просроченных ключей до 
значения ниже 25%. Этот подход необходим для предотвращения избыточного использования памяти для просроченных ключей и обычно не 
вызывает проблем, т.к. маловероятно, чтобы большое количество ключей истекали в одну и ту же секунду, но теоретически возможно, что 
пользователь массово применял команду expireat с одним и тем же значением времени. Поэтому важно понимать, что большое количество 
ключей, истекающих в один и тот же момент времени, может быть источником задержек.


Поэтому резюмируя, в данном случае редис блокируется потому что память занята истекшими ключами, но еще не удаленными. И алгоритм 
редиса тормозит чтобы не положить сервер по памяти.
...
Дополнение на вопрос:
Какие параметры конфига Postgres вы бы настроили, чтобы решить проблему с памятью?

Параметры памяти описаны здесь 
https://www.postgresql.org/docs/current/runtime-config-resource.html

Я бы попробовал поменять shared_buffers который  определяет количество памяти, выделенное для кэширования данных в оперативной памяти. 
Обычно рекомендуется выделить 25-35% от доступной памяти сервера..
И судя по рекомендациям  нельзя переборщить и вообще настройка параметров конфигурации PostgreSQL в идеале  должна производиться DBA.

Вообще надо смотреть еще по логам и там может быть что то конкретное есть и понятное что он выполнял перед тем как упасть , может и 
work_mem добавить или maintenance_work_mem . Потому что там может какой то DDL скрипт выполнялся или еще что то.
