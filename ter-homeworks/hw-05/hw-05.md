...
-migrate-state
```
Важно!!! При миграции пришлось мне создать БД YDB с таким же именем как и бакет в object storage , без этого не получалось
Поэтому я создал 
```
tfstate-develop-my
``` 
...
Также получил вот такуюд ошибку
```sh
